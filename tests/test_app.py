...
class AppTestCase(unittest.TestCase):
    def setUp(self):
        ...
        # registered first so it runs last, once the window is gone
    ...
    def _destroy_app(self):
        # only Tk teardown noise is ignored; a real shutdown fault must surface
        try:
            self.app.on_close()
        except tk.TclError:
            pass
    ...
    def test_translation_updates_output_and_history(self):
        ...
        self.assertEqual(str(self.app.translate_btn.cget("state")), "normal")
    ...
    def test_history_window_is_not_opened_twice(self):
        ...
        self.app.open_history()
        self.app.update()
        self.assertEqual(len(self._open_toplevels()), 1)
        for widget in self._open_toplevels():
            widget.destroy()
        self.app.update()
        # closing it must not prevent it from being opened again
        self.app.open_history()
        self.app.update()
        self.assertEqual(len(self._open_toplevels()), 1)
        for widget in self._open_toplevels():
    ...
    def test_history_restore_handles_an_unavailable_language(self):
        # history saved with the full online list may name a language this run lacks
        entry = {
            "timestamp": "2026-01-01 00:00:00", "source": "Dutch (nl)",
            "target": "Dutch (nl)", "original": "hello there friend",
            "translated": "hallo", "confidence": 90, "noun_mode": False,
        }
        self.app._use_history_entry(entry)
        self.app.update()
        self.assertEqual(self.app.source_combo.get(), "Auto Detect")
        self.assertIn(self.app.target_combo.get(), self.app.name_to_code)
        self.assertIn("unavailable", self.app.detected_lang_label.cget("text"))

    def test_unknown_target_language_reports_instead_of_doing_nothing(self):
        self.app.target_combo.set("Klingon")
        self.app._set_input("hello")
        self.app.translate()
        self.app.update()
        self.assertIn("not an available target", self.app.output_box.get("1.0", "end-1c"))

    def test_history_with_no_entries_tells_the_user(self):
        self.assertTrue(self.app.history.is_empty())
        self.app.open_history()
        self.app.update()
        self.assertEqual(self._open_toplevels(), [])
        self.assertIn("No translations saved", self.app.detected_lang_label.cget("text"))

    def test_open_history_window_updates_when_a_translation_finishes(self):
        self.app.history.add({
            "timestamp": "2026-01-01 00:00:00", "source": "English (en)",
            "target": "Spanish (es)", "original": "old", "translated": "viejo",
            "confidence": 90, "noun_mode": False,
        })
        self.app.open_history()
        self.app.update()
        window = self.app._history_window
        self.assertEqual(window.status_label.cget("text"), "Entries: 1")
        self.app.source_combo.set("English")
        self.app.target_combo.set("Spanish")
        self.app._set_input("hello")
        self.app.translate()
        self.assertTrue(self._pump_until(
            lambda: window.status_label.cget("text") == "Entries: 2"))
        window.destroy()
        self.app.update()

    def test_grammar_tooltip_is_not_rebuilt_for_the_same_message(self):
        # mouse motion repeats rapidly, so the window must not be recreated each time
        tooltip = self.app.tooltip
        tooltip.show(10, 10, "Verb error")
        self.app.update()
        first = tooltip._window
        for _ in range(5):
            tooltip.show(12, 10, "Verb error")
            self.app.update()
        self.assertIs(tooltip._window, first)
        tooltip.show(20, 10, "A different mistake")
        self.app.update()
        self.assertIsNot(tooltip._window, first)
        tooltip.clear()
        self.app.update()
        self.assertIsNone(tooltip._window)

    def test_toggle_noun_mode(self):
        before = self.app.noun_var.get()
        self.app.toggle_noun_mode()
        self.assertNotEqual(self.app.noun_var.get(), before)

    def test_shortcuts_do_not_corrupt_the_input_text(self):
        # Tk's own Text bindings would otherwise transpose or delete characters
        self.app.deiconify()
        self.app._set_input("abcd")
        self.app.input_box.focus_force()
        self.app.update()
        self.app.input_box.mark_set("insert", "end")
        for sequence in ("<Control-t>", "<Control-h>"):
            self.app.input_box.event_generate(sequence)
            self.app.update()
        self.assertEqual(self.app.input_box.get("1.0", "end-1c"), "abcd")
