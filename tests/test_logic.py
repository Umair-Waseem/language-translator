...
from translator_app.config import NOUN_MODE_MAX_CHARS
...
class DetectionTests(unittest.TestCase):
    ...
    def test_name_missing_from_the_map_falls_back_to_the_code(self):
        # the offline language list is small, so a detected name may not be in it
        result = detection.detect("Dit is een Nederlandse zin die duidelijk is", CODE_TO_NAME)
        self.assertEqual(result.code, "nl")
        self.assertIn("nl", result.message)
...
def make_entry(original="hi", translated="hola", confidence=90):
    # a record shaped exactly like the ones the application stores
    return {
...
class NounModeTests(unittest.TestCase):
    # noun mode is only worth attempting when both languages have a context phrase
    NOUN_VALID = {"en", "es", "nl"}

    def setUp(self):
        self.calls = []
        patcher = mock.patch.object(translation, "_translate_once", self._fake_translate)
        patcher.start()
        self.addCleanup(patcher.stop)

    def _fake_translate(self, text, source, target):
        self.calls.append(text)
        if text.startswith("Translate this noun:"):
            return "Traduce este sustantivo: gato"
        return "plain-gato"

    def test_supported_pair_strips_the_context(self):
        result, _, _ = translation.translate("cat", "en", "es", self.NOUN_VALID, noun_mode=True)
        self.assertEqual(result, "gato")
        self.assertEqual(len(self.calls), 1)

    def test_unsupported_target_translates_directly(self):
        result, _, _ = translation.translate("cat", "en", "nl", self.NOUN_VALID, noun_mode=True)
        self.assertEqual(result, "plain-gato")
        self.assertEqual(self.calls, ["cat"])

    def test_unsupported_source_translates_directly(self):
        translation.translate("cat", "nl", "es", self.NOUN_VALID, noun_mode=True)
        self.assertEqual(self.calls, ["cat"])

    def test_unknown_source_translates_directly(self):
        translation.translate("cat", "auto", "es", self.NOUN_VALID, noun_mode=True)
        self.assertEqual(self.calls, ["cat"])

    def test_long_text_ignores_noun_mode(self):
        long_text = "a" * (NOUN_MODE_MAX_CHARS + 1)
        translation.translate(long_text, "en", "es", self.NOUN_VALID, noun_mode=True)
        self.assertEqual(self.calls, [long_text])


class HistoryTests(unittest.TestCase):
    def setUp(self):
        # mkdtemp avoids the race in mktemp and is removed even if a test fails
        self._dir = tempfile.mkdtemp()
        self.addCleanup(shutil.rmtree, self._dir, ignore_errors=True)
        self.path = os.path.join(self._dir, "history.json")
    ...
    def test_add_and_trim_to_max_size(self):
        for i in range(4):
            self.store.add(make_entry(original=str(i)))
        self.assertEqual([e["original"] for e in self.store.snapshot()], ["1", "2", "3"])

    def test_persist_and_reload(self):
        self.store.add(make_entry(original="a"))
        reloaded = HistoryStore(path=self.path, max_size=3)
        reloaded.load()
        self.assertEqual([e["original"] for e in reloaded.snapshot()], ["a"])

    def test_delete_entry_by_value(self):
        a, b = make_entry(original="a"), make_entry(original="b")
        self.store.add(a)
        self.store.add(b)
        self.store.delete_entry(a)
        self.assertEqual([e["original"] for e in self.store.snapshot()], ["b"])

    def test_delete_missing_entry_is_noop(self):
        self.store.add(make_entry(original="a"))
        self.store.delete_entry(make_entry(original="does-not-exist"))
    ...
    def test_snapshot_is_a_copy(self):
        ...
        snap.append(make_entry(original="b"))
        self.assertEqual(len(self.store.snapshot()), 1)

    def test_add_refuses_a_malformed_record(self):
        # what add stores must always be reloadable later
        self.store.add({"original": "missing keys"})
        self.assertTrue(self.store.is_empty())
    ...
    def test_load_enforces_max_size(self):
        ...
        with open(self.path, "w", encoding="utf-8") as f:
            json.dump([make_entry(original=str(i)) for i in range(10)], f)
        self.store.load()
        self.assertEqual([e["original"] for e in self.store.snapshot()], ["7", "8", "9"])

    def test_load_drops_damaged_records(self):
        # one bad record must not make the whole history unusable
        good = {"timestamp": "t", "source": "English (en)", "target": "Spanish (es)",
                "original": "hi", "translated": "hola", "confidence": 90, "noun_mode": False}
        with open(self.path, "w", encoding="utf-8") as f:
            json.dump([good, {"original": "missing keys"}, "a bare string", 42], f)
        self.store.load()
        self.assertEqual([e["original"] for e in self.store.snapshot()], ["hi"])

    def test_load_normalizes_a_non_numeric_confidence(self):
        entry = {"timestamp": "t", "source": "English (en)", "target": "Spanish (es)",
                 "original": "hi", "translated": "hola", "confidence": "bad", "noun_mode": "yes"}
        with open(self.path, "w", encoding="utf-8") as f:
            json.dump([entry], f)
        self.store.load()
        stored = self.store.snapshot()[0]
        self.assertEqual(stored["confidence"], -1)
        self.assertIs(stored["noun_mode"], True)
