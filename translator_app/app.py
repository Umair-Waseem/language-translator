...
class TranslatorApp(ctk.CTk):
    def __init__(self):
        ...
        self._history_window = None  # the single open history window, if any
        self._closing = False
    ...
    def _bind_events(self):
        ...
        shortcuts = {
            "<Control-t>": self.translate,
            "<Control-s>": self.swap_languages,
            "<Control-l>": self.toggle_theme,
            "<Control-h>": self.open_history,
            "<Control-n>": self.toggle_noun_mode,
        }
        for sequence, action in shortcuts.items():
            self.bind(sequence, lambda e, fn=action: fn())
            # Tk edits the text on keys like Ctrl+T and Ctrl+H, so block it in the boxes
            for box in (self.input_box, self.output_box):
                box.bind(sequence, lambda e, fn=action: self._run_shortcut(fn))
    ...
    def _run_shortcut(self, action):
        # "break" stops Tk's built-in Text binding from also changing the text
        action()
        return "break"

    def toggle_noun_mode(self):
        self.noun_var.set(0 if self.noun_var.get() else 1)
    ...
    def translate(self):
        ...
        if target_name not in self.name_to_code:
            self._show_error(f"'{target_name}' is not an available target language")
            return
    ...
    def _on_grammar_motion(self, event):
        ...
        for match in matches:
            ...
            if widget.compare(index, ">=", start) and widget.compare(index, "<", end):
                ...
                return
        # the pointer is not over a mistake, so hide a tooltip that is still showing
        self.tooltip.clear()
    ...
    def _refresh_history_window(self):
        # an open window would otherwise keep showing the pre-translation list
        if self._history_window is not None and self._history_window.winfo_exists():
            self._history_window.refresh()

    def open_history(self):
        # a second window would steal the grab from the first, so reuse the open one
        if self._history_window is not None and self._history_window.winfo_exists():
            self._history_window.lift()
            self._history_window.focus_set()
            return
        if self.history.is_empty():
            self.detected_lang_label.configure(text="No translations saved yet")
            return
        self._history_window = HistoryWindow(self, self.history, self._use_history_entry)

    def _use_history_entry(self, entry):
        self._set_input(entry.get("original", ""))
        # names can contain "(...)", so split off only the trailing "(code)"
        source = entry.get("source", "").rsplit(" (", 1)[0]
        target = entry.get("target", "").rsplit(" (", 1)[0]
        # a stored language may be missing from the list available this session
        self.source_combo.set(source if source in self.name_to_code else AUTO_DETECT)
        if target in self.name_to_code:
            self.target_combo.set(target)
        self.noun_var.set(1 if entry.get("noun_mode") else 0)
        self.detect_language()
        if target and target not in self.name_to_code:
            self.detected_lang_label.configure(
                text=f"{target} is unavailable, target left as {self.target_combo.get()}")
