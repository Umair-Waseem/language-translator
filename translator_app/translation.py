...
def translate(text, source_code, target_code, valid_codes, noun_mode=False):
    ...
    if noun_mode and len(text) < NOUN_MODE_MAX_CHARS and _has_context(source, target):
...
def _has_context(source, target):
    # noun mode needs a phrase to build in the source and one to strip in the target
    return (_base_lang(source) in NOUN_CONTEXT_PHRASES
            and _base_lang(target) in NOUN_CONTEXT_PHRASES)


def _translate_noun(text, source, target):
    # bias short-word translations by embedding the word in a context phrase
    source_phrase = NOUN_CONTEXT_PHRASES[_base_lang(source)]
    translated = _translate_once(source_phrase.format(text=text), source, target)

    prefix = NOUN_CONTEXT_PHRASES[_base_lang(target)].split("{text}")[0].strip()
